...
class Solution{
    public int countVertex(int N, int[][] edges){
        ...
        return Math.min(f(graph,1,new byte[N+1],0,-1,new Integer[N+1][2]),f(graph,1,new byte[N+1],1,-1,new Integer[N+1][2])+1);
    }
    
    public static int f(ArrayList<Integer>[] graph,int src,byte[] visit,int isLighted,int prnt,Integer[][] dp) {
        if(dp[src][isLighted]!=null) return dp[src][isLighted];
        int ans=0;
        for(int nbr : graph[src]) {
            if(nbr!=prnt) {
                if(isLighted==0) {
                    int f1 = f(graph,nbr,visit,1,src,dp);
                    ans+=f1+1;
                } else {
                    int f0 = f(graph,nbr,visit,0,src,dp);
                    int f1 = f(graph,nbr,visit,1,src,dp);
